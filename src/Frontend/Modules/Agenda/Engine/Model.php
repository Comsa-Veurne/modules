...
class Model
{
    ...
    public static function get($URL)
    {
        $item = (array)FrontendModel::getContainer()->get('database')->getRecord(
            'SELECT i.id, i.language, i.title, i.introduction, i.text, i.allow_subscriptions, i.num_subscriptions, i.price,
        ...
        if (date('Y-m-d', $item['begin_date']) == date('Y-m-d', $item['end_date'])) {
            $item['same_day'] = true;
        }
     ...
     * @return array
     */
    public static function getAllUpcomingAgendaItems($start = 0, $limit = 9999)
    {
        ...
        if ($limit == null) {
            $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
                'SELECT i.*, UNIX_TIMESTAMP(i.begin_date) AS begin_date, UNIX_TIMESTAMP(end_date) AS end_date, m.url
                    ...
                    INNER JOIN meta AS m ON i.meta_id = m.id
                    WHERE i.language = ?
                    AND i.begin_date > DATE_SUB(NOW(), INTERVAL 1 DAY)
            ...
            $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
                ...
                array(FRONTEND_LANGUAGE, (int)$start, (int)$limit)
            );
        }
    ...
    public static function getAllCategories()
    {
        $return = (array)FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT c.id, c.title AS label, m.url, COUNT(c.id) AS total, m.data AS meta_data
			 ...
			 INNER JOIN agenda AS i ON c.id = i.category_id AND c.language = ?
        ...
        if ($notifyByMailOnSubscription) {
            ...
            if ($subscription['status'] == 'moderation') {
                $message = \Common\Mailer\Message::newInstance(FL::msg('NotificationSubject'))
                    ->parseHtml(
                        ...
                        array('message' => vsprintf(FL::msg('AgendaEmailNotificationsNewSubscriptionToModerate'),
            ...
            elseif ($subscription['status'] == 'published') {
                $message = \Common\Mailer\Message::newInstance(FL::msg('NotificationSubject'))
                    ->parseHtml(
                        FRONTEND_CORE_PATH . '/Layout/Templates/Mails/Notification.tpl',
                        array('message' => vsprintf(FL::msg('AgendaEmailNotificationsNewSubscription'),
