...
class Model implements FrontendTagsInterface
{
        ...
        if (!$id) {
            $item = (array)FrontendModel::getContainer()->get('database')->getRecord(
                'SELECT i.*, l.*,
				 ...
				 m.title AS meta_title, m.title_overwrite AS meta_title_overwrite, l.url, cl.url AS category_url, cl.title AS category_title, i.ballcolor
				 ...
				 INNER JOIN catalog_categories_lang AS cl ON i.category_id = cl.id
				 INNER JOIN meta AS m2 ON c.meta_id = m2.id
				 WHERE l.url = ? ', array(FRONTEND_LANGUAGE, (string)$url)
            );
        } else {
            $item = (array)FrontendModel::getContainer()->get('database')->getRecord(
                'SELECT i.*,
				 m.keywords AS meta_keywords, m.keywords_overwrite AS meta_keywords_overwrite,
				 m.description AS meta_description, m.description_overwrite AS meta_description_overwrite,
				 m.title AS meta_title, m.title_overwrite AS meta_title_overwrite, m.url, m2.url AS category_url, m2.title AS category_title, l.balltext, i.ballcolor
				 FROM catalog_products AS i
				 INNER JOIN catalog_products_lang AS l ON l.id = i.id AND l.language = ?
				 INNER JOIN meta AS m ON i.meta_id = m.id
				 INNER JOIN catalog_categories AS c ON i.category_id = c.id
				 INNER JOIN meta AS m2 ON c.meta_id = m2.id
				 WHERE i.id = ?', array(FRONTEND_LANGUAGE, (int)$id)
            );
        }
    ...
    public static function getProductSpecifications($id)
    {
        $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT i.*, l.*, c.value
			 ...
			 INNER JOIN catalog_specifications_lang AS l ON l.id = i.id AND l.language = ?
			 INNER JOIN catalog_specifications_values AS c ON i.id = c.specification_id
			 WHERE c.product_id = ? AND c.language = ?', array(FRONTEND_LANGUAGE, (int)$id, FRONTEND_LANGUAGE)
        );
        if (!empty($items)) {
    ...
    public static function getAll($limit = 10, $offset = 0)
    {
        $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT i.*,l.*
			 ...
			 INNER JOIN catalog_products_lang AS l ON l.id = i.id AND l.language = ?
			 INNER JOIN meta AS m ON i.meta_id = m.id
			 ORDER BY i.sequence ASC, i.created_on ASC, i.id DESC LIMIT ?, ?', array(FRONTEND_LANGUAGE, (int)$offset, (int)$limit)
        );
     ...
     * @return array
     */
    public static function getAllByCategory($categoryId, $limit = 9999, $offset = 0)
    {
        $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT i.*,l.*
			 FROM catalog_products AS i
			 ...
			 INNER JOIN meta AS m ON i.meta_id = m.id
			 WHERE i.category_id = ?
			 ORDER BY i.sequence ASC, i.id DESC LIMIT ?, ?', array(FRONTEND_LANGUAGE, $categoryId, (int)$offset, (int)$limit)
        );
        ...
        foreach ($items as &$item) {
            ...
            if ($img) {
                ...
                $item['image'] = '/' . FRONTEND_FILES_URL . '/Modules/Catalog/Layout/Images/dummy.png';
            }
     ...
     * @return array
     */
    public static function getCategoriesTree($id = 0, $selectedId = 0)
    {
        $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT c.id AS id, c.title, c.ballcolor, m.url, l.*, COUNT(p.id) AS total, m.data AS meta_data, parent_id
				 FROM catalog_categories AS c
				 INNER JOIN meta AS m ON c.meta_id = m.id
				 INNER JOIN catalog_categories_lang AS l ON l.id = c.id AND l.language = ?
        ...
        foreach ($items as &$row) {
            ...
            if ($img) {
                $row['image'] = $img['image'];
                $row['thumbnail'] = FRONTEND_FILES_URL . '/catalog/categories/150x150/' . $img['image'];
            } else {
    ...
    public static function getAllCategories($id = 0, $url = null)
    {
        ...
        if ($id != 0) {
            $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
                'SELECT c.id, c.title, c.ballcolor, l.*, COUNT(p.id) AS total, m.data AS meta_data
				 FROM catalog_categories AS c
				 INNER JOIN catalog_categories_lang AS l ON l.id = c.id AND l.language = ?
				 INNER JOIN meta AS m ON c.meta_id = m.id
				 LEFT OUTER JOIN catalog_products AS p ON p.category_id = c.id
            ...
            // no category given
            $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
                'SELECT c.id, c.title, c.ballcolor, l.*, COUNT(p.id) AS total, m.data AS meta_data, parent_id
				 FROM catalog_categories AS c
				 INNER JOIN catalog_categories_lang AS l ON l.id = c.id AND l.language = ?
                 INNER JOIN meta AS m ON c.meta_id = m.id
				 LEFT OUTER JOIN catalog_products AS p ON p.category_id = c.id
    ...
    public static function getCategory($URL)
    {
        $item = (array)FrontendModel::getContainer()->get('database')->getRecord(
            'SELECT i.*,l.*,
			 ...
			 INNER JOIN catalog_categories_lang AS l ON l.id = i.id AND l.language = ?
			 INNER JOIN meta AS m ON i.meta_id = m.id
			 WHERE l.url = ?', array(FRONTEND_LANGUAGE, (string)$URL)
        );
    ...
    public static function notifyAdmin(array $comment)
    {
        ...
        $notifyByMailOnCommentToModerate = FrontendModel::get('fork.settings')->get(
            'Catalog', 'notify_by_email_on_new_comment_to_moderate', false
        );
        ...
        if ($notifyByMailOnComment) {
            ...
            if ($comment['status'] == 'moderation') {
                ...
                $variables['message'] = vsprintf(
                    FL::msg('CatalogEmailNotificationsNewCommentToModerate'), array($comment['author'], $URL, $comment['product_title'], $backendURL)
                );
            } elseif ($comment['status'] == 'published') {
                // comment was published
                $variables['message'] = vsprintf(
                    FL::msg('CatalogEmailNotificationsNewComment'), array($comment['author'], $URL, $comment['product_title'])
                );
            }
            ...
            $message = \Common\Mailer\Message::newInstance(FL::msg('NotificationSubject'))->setFrom(array($from['email'] => $from['name']))->setTo(array($to['email'] => $to['name']))->setReplyTo(array($replyTo['email'] => $replyTo['name']))->parseHtml(
                FRONTEND_CORE_PATH . '/Layout/Templates/Mails/Notification.tpl', $variables, true
            );
    ...
    {
        $items = (array)FrontendModel::getContainer()->get('database')->getRecords(
            'SELECT i.id, i.title,i.image, m.url, COUNT(p.id) AS total, m.data AS meta_data
				 FROM catalog_brands AS i
				 INNER JOIN meta AS m ON i.meta_id = m.id
