...
class Model
{
    const QRY_DATAGRID_BROWSE = 'SELECT i.id, i.category_id, l.title AS title, i.sequence
		 FROM catalog_products AS i
		 INNER JOIN catalog_products_lang AS l ON l.id = i.id
		 WHERE l.language = ?
		 ORDER BY i.sequence ASC';

    const QRY_DATAGRID_BROWSE_FOR_CATEGORY = 'SELECT
			i.id, i.category_id, l.title AS title, i.sequence,
			c.title AS category
		 FROM catalog_products AS i
		 INNER JOIN catalog_products_lang AS l ON l.id = i.id
		 INNER JOIN catalog_categories AS c ON i.category_id = c.id
		 WHERE i.category_id = ? AND l.language = ?
		 ORDER BY i.sequence ASC';

    const QRY_DATAGRID_BROWSE_COMMENTS = 'SELECT
		 	i.id, UNIX_TIMESTAMP(i.created_on) AS created_on, i.author, i.text,
		 	p.id AS product_id, l.title AS product_title, m.url AS product_url
    ...
    const QRY_DATAGRID_BROWSE_PRODUCTS_ORDER = 'SELECT l.title, c.price, o.*, m.url
		 FROM `catalog_orders_values` AS o
		 INNER JOIN `catalog_products` AS c ON o.product_id = c.id
         INNER JOIN catalog_products_lang AS l ON l.id = c.id
    ...
    const QRY_DATAGRID_BROWSE_SPECIFICATIONS = 'SELECT c.id, l.title AS specification, c.sequence
		 FROM catalog_specifications AS c
		 INNER JOIN catalog_specifications_lang AS l ON l.id = c.id AND l.language = ?
		 GROUP BY c.id
		 ORDER BY c.sequence ASC';
    ...
    public static $ballColorArray = array('CD563C' => 'Orange', 'fcc514' => 'Yellow', '3399CC' => 'Blue');
    ...
    public static function delete($id)
    {
        ...
        BackendModel::getContainer()->get('database')->delete('catalog_products_lang', 'id = ?', (int)$id);
    ...
    public static function deleteSpecification($id)
    {
        ...
        if (!empty($item)) {
            ...
            $db->delete('catalog_specifications_lang', 'id = ?', array((int)$id));
     ...
     * @return bool
     */
    public static function existsSpecificationValue($productId, $specificationId, $lang)
    {
        return (bool)BackendModel::getContainer()->get('database')->getVar(
            'SELECT 1
			 FROM catalog_specifications_values AS i
			 WHERE i.product_id = ? AND i.specification_id = ? AND i.language = ?
			 LIMIT 1', array((int)$productId, (int)$specificationId, $lang)
        );
    }
    ...
    public static function getLanguage($id, $lang)
    {
        return (array)BackendModel::getContainer()->get('database')->getRecord(
            'SELECT i.*
			 FROM catalog_products_lang AS i
			 WHERE i.id = ? AND language = ?', array((int)$id, $lang)
        );
    }
     ...
     * @return string
     */
    public static function getSpecificationValue($specificationId, $productId, $lang)
    {
        $db = BackendModel::getContainer()->get('database');
        ...
        $value = (array)$db->getRecord(
            'SELECT i.value
			 ...
			 WHERE i.specification_id = ? AND i.product_id = ? AND language = ?', array((int)$specificationId, (int)$productId, $lang)
        );
     ...
     * @return array
     */
    public static function getCategoryLanguage($id, $lang)
    {
        return (array)BackendModel::getContainer()->get('database')->getRecord(
            'SELECT i.*
			 FROM catalog_categories_lang AS i
			 WHERE i.id = ? AND i.language = ?', array((int)$id, $lang)
        );
    }
     ...
     * @return array
     */
    public static function getSpecificationLanguage($id, $lang)
    {
        return (array)BackendModel::getContainer()->get('database')->getRecord(
            'SELECT i.*
			 FROM catalog_specifications_lang AS i
			 WHERE i.id = ? AND i.language = ?', array((int)$id, $lang)
        );
    }
    ...
    public static function getSpecifications()
    {
        ...
        $items = (array)$db->getRecords(
            'SELECT i.id, l.title, i.type
			 FROM catalog_specifications AS i
			 INNER JOIN catalog_specifications_lang AS l ON i.id =  l.id AND l.language = ?
			 GROUP BY i.id
			 ORDER BY i.sequence', array(BL::getWorkingLanguage())
        );
     ...
     * @return string
     */
    public static function getURLLanguage($url, $id = null, $language = '')
    {
        $url = \SpoonFilter::urlise((string)$url);
        $db = BackendModel::getContainer()->get('database');
        ...
        if ($id === null) {
            ...
            if ((bool)$db->getVar(
                'SELECT 1
				 ...
				 LIMIT 1', array($language, $url)
            )
            ) {
                $url = BackendModel::addNumber($url);
                return self::getURLLanguage($url, null, $language);
            }
        } else {
            // current item should be excluded
            // already exists
            if ((bool)$db->getVar(
                'SELECT 1
				 ...
				 LIMIT 1', array($language, $url, $id)
            )
            ) {
                $url = BackendModel::addNumber($url);
                return self::getURLLanguage($url, $id, $language);
            }
        }
     ...
     * @return string
     */
    public static function getURLForCategoryLanguage($url, $id = null, $language)
    {
        $url = \SpoonFilter::urlise((string)$url);
        $db = BackendModel::getContainer()->get('database');
        ...
        if ($id === null) {
            if ((bool)$db->getVar(
                ...
                $url = BackendModel::addNumber($url);
                return self::getURLForCategoryLanguage($url, null, $language);
            }
        } else {
            // current category should be excluded
            if ((bool)$db->getVar(
                ...
                $url = BackendModel::addNumber($url);
                return self::getURLForCategoryLanguage($url, $id, $language);
            }
        }
     ...
     * @return int
     */
    public static function insertLanguage(array $item)
    {
        return (int)BackendModel::getContainer()->get('database')->insert('catalog_products_lang', $item);
    }
    ...
    public static function insertCategory(array $item)
    {
        ...
        BackendModel::updateExtra(
            $item['extra_id'], 'data', array('id' => $item['id'], 'extra_label' => BL::getLabel('Category') . ' ' . $item['title'], 'language' => $item['language'], 'edit_url' => BackendModel::createURLForAction(
                    'EditCategory', 'Catalog', $item['language']
                ) . '&id=' . $item['id'])
        );
    ...
    public static function insertCategoryLanguage(array $item)
    {
        BackendModel::getContainer()->get('database')->insert('catalog_categories_lang', $item);
    }
     ...
     * @return int
     */
    public static function insertSpecificationLanguage(array $item)
    {
        return BackendModel::getContainer()->get('database')->insert('catalog_specifications_lang', $item);
    }
     ...
     * @param array $item
     */
    public static function updateLanguage(array $item, $lang)
    {
        BackendModel::getContainer()->get('database')->update('catalog_products_lang', $item, 'id = ? AND language = ?', array((int)$item['id'], $lang));
    }
    ...
    public static function updateCategory(array $item)
    {
        ...
        // update extra
        BackendModel::updateExtra(
            $item['extra_id'], 'data', array('id' => $item['id'], 'extra_label' => BL::getLabel('Category') . ' ' . $item['title'], 'language' => $item['language'], 'edit_url' => BackendModel::createURLForAction('EditCategory') . '&id=' . $item['id'])
        );
    }
     ...
     * @param array $item
     */
    public static function updateCategoryLanguage(array $item)
    {

        BackendModel::getContainer()->get('database')->update('catalog_categories_lang', $item, 'id = ? AND language = ?', array($item['id'], $item['language']));
    }
     ...
     * @param array $item
     */
    public static function updateSpecificationLanguage($item, $lang)
    {
        BackendModel::getContainer()->get('database')->update('catalog_specifications_lang', $item, 'id = ? AND language = ?', array($item['id'], $lang));
    }
     ...
     * @param array $item
     */
    public static function updateSpecificationValue($specificationId, $productId, $lang, $item)
    {
        BackendModel::getContainer()->get('database')->update('catalog_specifications_values', $item, 'specification_id = ? AND product_id = ? AND language = ?', array((int)$specificationId, (int)$productId, $lang));
    }
