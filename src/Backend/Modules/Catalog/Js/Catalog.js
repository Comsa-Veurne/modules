...
jsBackend.catalog =
{
    ...
    init: function ()
    {
        ...
        jsBackend.catalog.specifications();
    ...
    {
        //--Get all the first input elements
        $("table.specifications input:first-child").blur(function ()
        {
            var name = $(this).attr('name');
            var value = $(this).val();
            name = name.slice(0, -3);
            $("table.specifications input[name^=" + name + "]").each(function (e, o)
            {
                if ($(o).val() == '')
                {
                    $(o).val(value);
