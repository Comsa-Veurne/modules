...
class Model
{
    ...
    const QRY_DATAGRID_BROWSE_SUBSCRIPTIONS =
        'SELECT i.id, UNIX_TIMESTAMP(i.created_on) AS created_on, i.name, a.title
		 FROM agenda_subscriptions AS i
		 INNER JOIN agenda AS a ON i.agenda_id = a.id
    ...
    public static function insert(array $item)
    {
        ...
        $item['id'] = (int)BackendModel::getContainer()->get('database')->insert('agenda', $item);

        $db = BackendModel::getContainer()->get('database');
        // build extra for the gallery-widget
        $extra = array('module' => 'Agenda', 'type' => 'widget', 'label' => 'AgendaPoint', 'action' => 'AgendaPoint', 'data' => serialize(array('id' => $item['id'], 'extra_label' => "Agenda " . $item['title'], 'language' => $item['language'])), 'hidden' => 'N', 'sequence' => $db->getVar(
        ...
        $update = $db->update('agenda', $item, 'id = ?', array($item['id']));
        return $item['id'];
    }
